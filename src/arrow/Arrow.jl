...
const UNIXEPOCH_TS = Dates.value(Dates.DateTime(1970)) #Rata Die milliseconds for 1970-01-01T00:00:00
...
function unix2datetime(::Type{P}, x) where P
    ...
    return Dates.DateTime(Dates.UTM(rata))
end
datetime2unix(x::Dates.DateTime) = Dates.value(x) - UNIXEPOCH_TS

Base.convert(::Type{Dates.DateTime}, x::Timestamp{P,Z}) where {P, Z} = unix2datetime(P, x)
Base.show(io::IO, x::Timestamp) = show(io, convert(Dates.DateTime,x))
